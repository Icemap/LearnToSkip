"""Distance-threshold baseline builder - real per-candidate approx distance skipping."""
import heapq
import math
import time
...
class DistanceThresholdBuilder(BaseBuilder):
    """Skip candidates whose approximate distance exceeds a dynamic threshold.

    For each candidate, compute approx_dist. If it exceeds the percentile-based
    threshold (calibrated from a sample), skip the exact distance computation.
    """

    def __init__(self, percentile: float = 70.0, seed: int = SEED) -> None:
        self.percentile = percentile
        self.seed = seed

    @property
    def name(self) -> str:
        return f"Dist-Threshold(p{int(self.percentile)})"
    ...
    def build(
        ...
        approx = RandomProjectionDistance(seed=self.seed)
        approx.fit(data)

        # Calibrate threshold from a sample of pairwise approx distances
        sample_idx = rng.choice(n, size=min(1000, n), replace=False)
        sample_dists = []
        for i in sample_idx[:100]:
            targets = rng.choice(n, size=50, replace=False)
            d = approx.estimate_batch(data[i], data[targets])
            sample_dists.extend(d.tolist())
        dist_threshold = float(np.percentile(sample_dists, self.percentile))
            ...
            for l in range(min(level, max_level), -1, -1):
                ...
                while candidates:
                    ...
                    for nb in neighbors[c].get(l, []):
                        ...
                        # Approx distance threshold skip
                        ad = approx.estimate(data[i], graph_data[nb])
                        if ad > dist_threshold:
